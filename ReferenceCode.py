# 조코딩 유튜브에서 개발한 레퍼런스 코드입니다.
# config.yaml, API 함수 등을 개발 과정에서 참고했습니다.
...
ACCESS_TOKEN = ""
TOKEN_ISSUE_TIME = None  # (Fe수정 5/14) 토큰 발급 시간 저장 변수
CANO = _cfg['CANO']
...
def get_access_token():
    """토큰 발급"""
    global ACCESS_TOKEN, TOKEN_ISSUE_TIME  # (Fe수정 5/14) 토큰 발급 시간 저장 변수 추가
    ...
    TOKEN_ISSUE_TIME = datetime.datetime.now()  # (Fe수정 5/14) 발급 시점 저장
    return ACCESS_TOKEN
...
def check_and_refresh_token():  # (Fe수정 5/14) 토큰 만료 방지용 함수 추가
    """토큰이 18시간 이상 되었으면 갱신"""
    global ACCESS_TOKEN, TOKEN_ISSUE_TIME
    if TOKEN_ISSUE_TIME is None:  # 토큰이 발급된 적이 없으면 새로 발급
        send_message("토큰이 없으므로 새로 발급합니다.")
        return get_access_token()
    
    elapsed_time = (datetime.datetime.now() - TOKEN_ISSUE_TIME).total_seconds() / 3600  # 경과 시간(시간 단위)
    
    if elapsed_time >= 18:  # 18시간 이상 경과했으면 새 토큰 발급
        send_message("토큰이 18시간 이상 경과하여 갱신합니다.")
        return get_access_token()
    return ACCESS_TOKEN  # 기존 토큰 유지
...
try:
    ...
    while True:
        ACCESS_TOKEN = check_and_refresh_token()  # (Fe수정 5/14) 루프에서 토큰 갱신 여부 확인
